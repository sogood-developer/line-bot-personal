...
class Liff:
    ...
    def sendLiff(self, to, data, mainType=True):
        ...
        if not mainType: data = {"type": "flex", "altText": "Sozi ßot", "contents": {"type": "carousel", "contents": data}}
    ...
    def sendLiffImage(self, to, link, icon, label, reply=False):
        ...
        if self.settings["templateMode"]:
            self.sendLiff(to, data)
            time.sleep(0.5)
        elif reply: self.sendReplyImageWithURL(to, link)
        else: self.sendImageWithURL(to, link)
    
    @loggedIn
    def sendBigSticker(self, to, contentMetadata={}):
        if 'STKOPT' in contentMetadata:
            if contentMetadata['STKOPT'] == 'A':
                url = 'https://stickershop.line-scdn.net/stickershop/v1/sticker/%s/ANDROID/sticker_animation@2x.png' % contentMetadata["STKID"]
                req = requests.get(url)
                if req.status_code != 200:
                    url = 'https://stickershop.line-scdn.net/stickershop/v1/sticker/%s/iPhone/sticker_animation@2x.png' % contentMetadata["STKID"]
            else:
                url = 'https://stickershop.line-scdn.net/stickershop/v1/sticker/%s/ANDROID/sticker.png' % contentMetadata["STKID"]
                req = requests.get(url)
                if req.status_code != 200:
                    url = 'https://stickershop.line-scdn.net/stickershop/v1/sticker/%s/iPhone/sticker@2x.png' % contentMetadata["STKID"]
        else:
            url = 'https://stickershop.line-scdn.net/stickershop/v1/sticker/%s/ANDROID/sticker.png' % contentMetadata["STKID"]
            req = requests.get(url)
            if req.status_code != 200:
        ...
        data = {
            'type': 'template',
            'altText': '%s sent a sticker' % self.profile.displayName,
            'template': {
                'type': 'image_carousel',
                'columns': [
                    {
                        ...
                        'action': {
                            'type': 'uri',
                            'uri': 'https://line.me/R/shop/detail/{}'.format(contentMetadata["STKPKGID"])
                        }
                    }
                ]
            }
